...
window.open('Advance_Report_HTML.jsp?adv_no='+b+'','mywindow','width=700,height=500,toolbar=no,menubar=no');
	...
	<div id="conteneur">
		...
		include("logo.php");
			...
			?>
		<div id="centre">
          <h1 style="color:red;" align="center">ABC Analysis</h1>
...
<tr height="25px"><TH>S.NO</TH><TH>PRODUCT NAME</TH><TH>SUM</TH><TH>ABC</TH></tr>
<?php 
			  include("config.php");
			   if(isset($_REQUEST['submit'])){
			  $d=$_REQUEST['name'];
			  
			  $sq = mysqli_query($link,"select a.product_code,a.product_name,sum(value) from phr_purinv_dtl as a,phr_prddetails_mast as b where a.PRODUCT_NAME=b.PRD_NAME and a.product_name ='$d' group by a.PRODUCT_NAME")or die(mysqli_error($link));
             ...
             <?php $i++;}
			  ...
			  $sq = mysqli_query($link,"select a.product_code,a.product_name,sum(value) from phr_purinv_dtl as a,phr_prddetails_mast as b where a.PRODUCT_NAME=b.PRD_NAME group by a.product_code")or die(mysqli_error($link));
			$tot=mysqli_num_rows($sq);
			  $i = 1;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<td width="25" align="right"><?php if (!($pagecount == 0)) { ?><a href="ABC_Analysis.php?next=0" ><?php } ?><img src="images/first.gif" title="First" alt="First" width="16" height="14" border="0"/></a></td>
		<td width="10">&nbsp;</td>
	   
		<td width="25" align="right"><?php if (!($pagecount == 0)) { ?>
			  <a  href="ABC_Analysis.php?next=<?php echo ($pagecount - 1) ?>"><?php } ?><img src="images/previous.gif" title="Previous" alt="Previous" width="16" height="14" border="0"/></a></td>
		  <td width="10">&nbsp;</td>
	  
		<td width="25" align="right"><?php if ($rowscounts > 0) { ?>
			  <a href="ABC_Analysis.php?next=<?php echo ($pagecount + 1) ?>"><?php } ?><img src="images/next.gif" title="Next" alt="Next" width="16" height="14" border="0"/></a></td>
		<td width="10">&nbsp;</td>
		<td width="25" align="right"><?php if ($rowscounts > 0) { ?><a href="ABC_Analysis.php?next=<?php echo (($records - 1) / $nd) ?>" >  <?php } ?><img src="images/last.gif" title="Last" alt="Last" width="16" height="14" border="0"/></a></td>
	  </tr>
	</table>
	<table>
	<?php if ($rowscounts == 0) { ?>
