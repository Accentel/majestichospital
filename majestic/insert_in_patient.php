...
$tot = (float)$amt- (float)$conamt+ (float)$mrcost+ (float)$dcost+ (float)$roomrents;
 $bal=$tot-$advamt; 
$adt=$_POST['admitdate'];
$pname=$_POST['pname'];
...
$sq="insert into ip_pat_admit(PAT_REGNO,PAT_NO,ADMIT_DATE,ADMIT_TIME,BED_NO,DIS_STATUS,BILL_STATUS,CONCESSION_TYPE,CONCESSION_CARDNO,AMOUNT,CONS_AMT,ALLOT_BY,CURRENTDATE,AUTH_BY,doc_code,cash_credit,diet_cost,MR_cost,package,bill_num,adv_amnt,bal,tot) 
	values('$pregno','$pno','$adate',str_to_date('$atime','%r'),'$bedno','ADMITTED','NOT PAID','$ctype','$concardno','$amt','$conamt','$allotby',now(),'$aname','$doccode','$cashcredit','$dcost','$mrcost','$pkg','$cnt','$advamt','$bal','$tot')";
 } 

 else {
	 
	$sq="insert into ip_pat_admit(PAT_REGNO,PAT_NO,ADMIT_DATE,ADMIT_TIME,BED_NO,DIS_STATUS,BILL_STATUS,CONCESSION_TYPE,CONCESSION_CARDNO,AMOUNT,CONS_AMT,ALLOT_BY,CURRENTDATE,AUTH_BY,doc_code,cash_credit,diet_cost,MR_cost,package,bill_num,adv_amnt,bal,tot) 
	values('$pregno','$pno','$adate',str_to_date('$atime','%r'),'$bedno','ADMITTED','NOT PAID','$ctype','$concardno','$amt','$conamt','$allotby',now(),'$aname','$doccode','$cashcredit','$dcost','$mrcost','$pkg','','$advamt','$bal','$tot')";
