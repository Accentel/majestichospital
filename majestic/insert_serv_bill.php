...
//echo $q="insert into bill1(BillNO,PatientName,Total,Discount,NetAmount,PaidAmount,BalanceAmount,user) values('$bno','$pname','$total','$disc','$namt','$paid','$bal')";
 $q="insert into sbill1( PatientName, Age, Sex, DoctorName,Total,Discount,NetAmount,PaidAmount,BalanceAmount,BillDate,user,time,mrno,ptype,sample,paymenttype,cnt) values('$pname','$age','$gender','$dname','$total','$disc','$namt','$paid','$bal','$bdate','$user','$time','$mrno','$ptype','$sampleno','$paymenttype','$cnt')";
...
$dt = date('d-m-Y');
			...
			$str = "BL-".$dt3."-";
			$sam = "SN-".$dt3."-";
			$sql = mysqli_query($link,"select count(distinct BillNO) from sbill where BillNO like '$str%'")or die(mysqli_error($link));
			if($sql){
		 ...
		 $uy="update sbill1 set BillNo='$bno',sample='$sno' where bid='$id'";
...
$serv_name = $_POST['serv_name'][$i];
$amount = $_POST['amount'][$i];
if($serv_name != ""){
$sql1 = mysqli_query($link,"insert into sbill(BillNO, BillDate, PatientName, Age, Sex, DoctorName,serv_name,Amount,Total,Discount,NetAmount,PaidAmount,BalanceAmount,conce_type,ptype,remarks,user,phoneno,mrno) values('$bno','$bdate','$pname','$age','$gender','$dname','$serv_name','$amount','$total','$disc','$namt','$paid','$bal','$ctype','$ptype','$remarks','$user','$mno','$mrno')");
}

}


//$qq=mysqli_query($link,"insert into daily_amount(amnt_type,amount,bill_num,mrnum,recv_by,pay_type,date_time,amnt_desc)values('Service','$paid','$cnt','$mrno','$user','$paymenttype','$ndate','Service')");
...
if($sql1)
{
	...
	print "self.location='service_bill.php';";
	print "</script>";
	header("Location:sbill_rec2.php?bno=$bno");
