...
//$abc=$_GET['id'];
	...
	$a=("select max(PAT_ID)+0 from ip_pat_admit");
	$abc = mysqli_query($link, $a) or die(mysqli_error($link));
...
window.onload=GetClock;
		  ...
		  $t = ("select * from patientregister where reg_id='$id'");
		
		  $q = mysqli_query($link, $t) or die($link);
...
<?php
	...
	$l = ("select * from doct_infor");
	$sql = mysqli_query($link, $l) or die(mysqli_error($link));
...
$s1 = ("select CONCE_CODE, CONCE_NAME from concession_type");
$sql1 = mysqli_query($link, $s1) or die(mysqli_error($link));
...
<tr>
  ...
  <?php
	...
	$sl = ("select BED_NO from bed_details where bed_status='Unreserved'");
	
	$sql = mysqli_query($link, $sl) or die(mysqli_error($link));
